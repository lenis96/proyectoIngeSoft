...
public class ModeloGestionInmuebles extends DataBaseConnection{
	...
	public DefaultTableModel obtenerInmuebles(){
		...
		int registros=0;
		String[] columNames={"ID","DIRECCION","PRECIO","OTROS"};
		try{
		...
		try{
			PreparedStatement statement=getConnection().prepareStatement("select id,direccion,precio,otro from inmuebles");
			ResultSet respuesta=statement.executeQuery();
			int i=0;
			while(respuesta.next()){
				...
				data[i][3]=respuesta.getString("otro");
				i++;
			}
			respuesta.close();
