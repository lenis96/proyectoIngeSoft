/*************************************************

Nombre de la clase: ControladorCrearInventario.java

Última modificación: 06/06/2016

Descripción: Controla el despliegue y funcionamiento
	de la interfaz para agregar un nuevo inventario a
	la colección.

*************************************************/
import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;
import javax.swing.*;

public class ControladorCrearInventario extends JDialog implements ActionListener{
	private int idInmueble;
	private String[] cantidades={"0","1","2","3","4","5"};
	private JButton crearInventarioBoton=new JButton("crear Inventario");
	private ModeloInventarios modelo;
	private JComboBox[] espaciosInputs;
	public ControladorCrearInventario(JDialog parent,int idInmueble,ModeloInventarios modelo) {
		super(parent);
		this.modelo=modelo;
		this.idInmueble=idInmueble;
		setSize(400,400);
		setLocation(100, 100);
		setLayout(null);
		
		JLabel label=new JLabel("id: "+String.valueOf(idInmueble)+"  direccion: "+modelo.getDireccionInmueble(idInmueble));
		label.setSize(label.getPreferredSize());
		label.setLocation(10, 10);
		add(label);
		
		String[] espacios=modelo.obtenerEspaciosDefault();
		espaciosInputs=new JComboBox[espacios.length];
		int i;
		for(i=0;i<espacios.length;i++){
			label=new JLabel("numero de "+espacios[i]+":");
			label.setSize(label.getPreferredSize());
			label.setLocation(10, (i*30)+50);
			add(label);
			espaciosInputs[i]=new JComboBox(cantidades);
			espaciosInputs[i].setSize(espaciosInputs[i].getPreferredSize());
			espaciosInputs[i].setLocation(160, (i*30)+50);
			add(espaciosInputs[i]);
		}
		
		crearInventarioBoton.setSize(crearInventarioBoton.getPreferredSize());
		crearInventarioBoton.setLocation(10, (i*30)+70);
		crearInventarioBoton.addActionListener(this);
		add(crearInventarioBoton);

		setVisible(true);
		
	}
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==crearInventarioBoton){
			String[] cantidades=new String[espaciosInputs.length];
			for(int i=0;i<espaciosInputs.length;i++){
				cantidades[i]=espaciosInputs[i].getSelectedItem().toString();
			}
			if(modelo.crearInventarios(idInmueble, cantidades)){
				JOptionPane.showMessageDialog(this, "Se pudo crear el nuevo inventario");
			}
			else{
				JOptionPane.showMessageDialog(this, "No se pudo crear el nuevo inventario");
			}
			dispose();
		}
		
	}

}
