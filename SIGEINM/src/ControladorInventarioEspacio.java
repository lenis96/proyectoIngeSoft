...
Nombre de la clase: ControladorInventarioEspacio.java

Última modificación: 06/06/2016

Descripción: Controla el despliegue y funcionamiento
	de la interfaz para editar la información de un
...
public class ControladorInventarioEspacio extends JDialog implements ActionListener {
	...
	int instancia;
	int idInventario;
	public ControladorInventarioEspacio(JDialog parent,ModeloInventarios modelo,int idInmueble,int idInventario,int idEspacio,int instancia,String modo,String tipo) {
		super(parent);
		this.modelo=modelo;
		this.modo=modo;
		...
		panel.add(label);
		System.out.println("kkkkkkkkk"+idInventario);
		String[][] informacion=modelo.obtenerInformacionObjetos(modelo.getEspaciosInventarioId(idInventario, idEspacio, instancia));
		...
		if(modo.equals("E")){
			...
			indicesObjetos=modelo.obtenerIndicesObjetos(modelo.getEspaciosInventarioId(idInventario, idEspacio, instancia));
			System.out.println("abajo");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==guardar){
			int idEspacioInventario=modelo.getEspaciosInventarioId(idInventario, idEspacio, instancia);
			if(modo.equals("G")){
				...
				if(modelo.guardarEspacio(indicesObjetos,estados,indicesMateriales,cantidades,observaciones,idEspacioInventario)){
					...
					modelo.desAprovarInventario(idInventario);
				}
				else{
				...
				if(modelo.Actualizar(indicesObjetos,estados,indicesMateriales,cantidades,observaciones,idEspacioInventario)){
					JOptionPane.showMessageDialog(this,"se pudo actualizar el inventario exitosamente");
					modelo.desAprovarInventario(idInventario);
				}
				else{
					JOptionPane.showMessageDialog(this, "no se pudo actualizar la informacion");
				}
			}
		}
	}
