...
public class ModeloInventarios extends DataBaseConnection{
	...
	public DefaultTableModel obtenerInventarios(boolean todos){
		/*
		 * Funcion que entre un DefaultModel dado el usuario.
		 * Si el usuario es tipo admintrador o secretaria le va a retornar todos los inventarios
		 * 
		 */
	...
	int getIdEspacio(String espacio){
		/*
		 * Funcion que entrega el id del espacio dado es espacio ingresado
		 */
		int idEspacio=0;
		try{
	...
	boolean getAprovacion(int idInventario){
		/*
		 * Funcion que retorna si el inventario a sido aprovado.
		 */
		try{
	...
	boolean aprovarInventario(int idInventario){
		/*
		 * Funcion que dado el id del inventario manda a aprovarlo
		 */
		try{
	...
	boolean desAprovarInventario(int idInventario){
		/*
		 * Funcion que dado el id del inventario manda a desaprovarlo
		 */
		try{
	...
	boolean InformacionCreadaInventario(int idInventario,int idEspacio,int instancia){
		/*
		 * retorna true si la informacion del inventario ha sido creada de lo contario false
		 */
		try{
	...
	boolean inventarioCreado(int idInmueble){
		/*
		 * Funcion que retorna si se ha resevado el id del inventario
		 */
		try{
	...
	boolean crearInventarios(int idInmueble,String[] cantidades){
		/*
		 * Crea el inventario con sus respectivos espacios.
		 */
		try{
	...
	String getDireccionInmueble(int idInmueble){
		/*
		 * Funcion que entrega la direccion del inmueble dado el id del inmueble.
		 */
		String direccion="";
		try{
	...
	String getNumHabitaciones(int idInmueble){
		/*
		 * Funcion que entrega el numerp de habitaces tiene el inmueble
		 */
		String habitaciones="";
		try{
	...
	String [] obtenerEspaciosDefault(){
		/*
		 * Funcion que retorna los espacios que se encuentran en la base de datos.
		 */
		String[] espacios;
		int registros=0;
		try{
	...
	boolean agragarObjeto(String objeto){
		...
		statement.execute();
		statement=getConnection().prepareStatement("select id from espaciosdefault");
		ResultSet respuesta=statement.executeQuery();
		...
		idObjetoRespuesta.next();
		int idObjeto=idObjetoRespuesta.getInt(1);
		while(respuesta.next()){
			statement=getConnection().prepareStatement("insert into objetosDefault values(?,?)");
			statement.setInt(1, idObjeto);
			statement.setInt(2, respuesta.getInt(1));
			statement.execute();
		}
		return true;
		}catch(SQLException exception){
